...
Red_button_processor::Red_button_processor() :
        vsm::Request_processor("Red button processor")
{
    ...
    reset_request = vsm::Io_buffer::Create(reset_buf, sizeof(reset_buf));
    poll_request = vsm::Io_buffer::Create(poll_buf, sizeof(poll_buf));
}
...
Red_button_processor::On_disable()
{
    ...
    vsm::Request_processor::On_disable();
}
...
{
    vsm::Request::Ptr req = Create_listen_request(handler, ctx);
    Submit_request(req);
    return req;
}
                                            ...
                                            Event_id next_event)
{
    ...
    auto comp_handler =
        vsm::Make_callback(&Red_button_processor::Handle_event, this,
                           handler, ctx, next_event);
    req->Set_completion_handler(ctx, comp_handler);
    req->Set_processing_handler(
        vsm::Make_callback(&Red_button_processor::Handle_listen_request, this,
...
Red_button_processor::Read()
{
    rb_stream->Read(REPORT_SIZE, REPORT_SIZE,
                    vsm::Make_read_callback(&Red_button_processor::Read_handler, this),
                    comp_ctx);
}
...
Red_button_processor::Schedule_connect()
{
    ...
    connect_timer = vsm::Timer_processor::Get_instance()->Create_timer
...
{
    try {
        rb_stream = vsm::Hid_processor::Get_instance()->Open(VENDOR_ID, PRODUCT_ID);
        LOG("Red button connected");
        Reset();
        Read();
        Start_polling();
    } catch (vsm::Hid_processor::Exception &) {
        rb_stream = nullptr;
    }
}
...
Red_button_processor::Reset()
{
    ...
    } catch (vsm::Hid_processor::Invalid_state_exception &) {
        Disconnect();
    }
}
...
void
Red_button_processor::Fire_event_handler(Event_id event_id, vsm::Request::Ptr req,
                                         Event_handler handler)
{
    Event &event = event_queue[event_id & (EVENT_QUEUE_SIZE - 1)];
    vsm::Io_result result =
        event.state == State_code::INITIAL ? vsm::Io_result::CLOSED : vsm::Io_result::OK;
    bool pressed = event.state == State_code::PRESSED;

    auto lock = req->Lock();
    if (req->Is_processing()) {
        handler.Set_args(pressed, result);
        req->Complete(vsm::Request::Status::OK, std::move(lock));
    }
}

void
Red_button_processor::Handle_listen_request(vsm::Request::Ptr req,
                                            Event_handler handler,
                                            Event_id *next_event)
{
...
{
    if (handler.Invoke()) {
        vsm::Request::Ptr req = Create_listen_request(handler, ctx, last_event);
        Submit_request(req);
    }
}
...
void
Red_button_processor::Read_handler(vsm::Io_buffer::Ptr data, vsm::Io_result result)
{
